...
def chekcPass(username, password)->tuple:
    CLIENT.send_message(f"CHECKPASS,{username},{password}")
...
def checkNewUser(username, password, gender, bdate, type)->tuple:
    CLIENT.send_message(f"NEWUSER,{username},{password},{gender},{bdate},{type}")
    msg = CLIENT.get_messages()
...
def open_user_win(window, username:str):
    window.destroy()
...
def guest_screen():
    ...
    screen.title(f"Guest {CLIENT.name}")
...
def host_screen():
    screen = Toplevel(home_screen)
    screen.geometry("300x250")
    screen.title(f"Host {CLIENT.name}")
    create_menu_bar(screen)
    # CLIENT.send_message('USERTOSTRING')
    # msg = CLIENT.get_messages()
    # while len(msg) == 0 or msg[-1].split(',')[0] != 'USERTOSTRING':
